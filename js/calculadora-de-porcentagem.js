...
function corrigirValor() {
  ...
  if (!indicesCarregados) {
    mostrarToast('Indices temporariamente indisponíveis. Tente novamente mais tarde.');
    return;
  }
...
async function carregarIndices() {
  try {
    ...
    if (!res.ok) { indicesCarregados = false; return; }
    const dados = await res.json();
    if (dados.ipca) HIST_IPCA = dados.ipca;
    if (dados.igpm) HIST_IGPM = dados.igpm;
    if (dados.poupanca) HIST_POUP = dados.poupanca;
    if (dados.tr) HIST_TR = dados.tr;
    indicesCarregados = true;
  } catch (e) {
    indicesCarregados = false;
  }
}
